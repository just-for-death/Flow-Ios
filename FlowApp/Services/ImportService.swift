...
enum ImportService {
    ...
    static func importWatchHistoryDatabase(from url: URL) async throws -> Int {
        ...
        try dbQueue.read { db in
            ...
            for row in rows {
                ...
                let progress: Double = row["progress"] ?? 0
                let durationGuess = max(progress, 1)
                let pct = Float(min(1, progress / durationGuess))
    ...
    private static func unzipDB(from zipURL: URL, to dest: URL) throws {
        ...
        // Synchronous fallback: copy if already a db
        if zipURL.pathExtension == "db" {
    ...
    enum ImportError: Error, LocalizedError {
        ...
        var errorDescription: String? { "ZIP import requires extracting newpipe.db manually on iOS for now." }
    }
}
